...
module cpu (
    ...
    always_ff @(posedge clk) begin
        if (reset) begin
            ...
            pc <= inst.op == JNZ && reg_1 != '0 ? reg_2[7:0] : pc + 1'b1; // New!
            case (inst.op)
                MOVE: regs[inst.dst] <= reg_1;
                LOAD: regs[inst.dst] <= dmem_rdata;
                ADD:  regs[inst.dst] <= reg_1 + reg_2;
                SUB:  regs[inst.dst] <= reg_1 - reg_2;
                MUL:  regs[inst.dst] <= reg_1 * reg_2;
                default: ;
            endcase
        end
    end
